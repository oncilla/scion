...
func PollReqHdlr() {
	...
	for rpld := range disp.Dispatcher.PollReqC {
		...
		if err != nil {
			log.Error("PollReqHdlr: Down creating SIGCtrl payload", "err", err)
			break
		}
		cpld, err := ctrl.NewPld(spld, nil)
		if err != nil {
			log.Error("PollReqHdlr: Down creating Ctrl payload", "err", err)
			break
		}
		scpld, err := cpld.SignedPld(ctrl.NullSigner)
		if err != nil {
			log.Error("PollReqHdlr: Down creating signed Ctrl payload", "err", err)
			break
		}
		raw, err := scpld.PackPld()
		if err != nil {
			log.Error("PollReqHdlr: Down packing signed Ctrl payload", "err", err)
			break
		}
		...
		if err != nil {
			log.Error("PollReqHdlr: Down sending Ctrl payload", "dest", rpld.Addr, "err", err)
		}
	}
