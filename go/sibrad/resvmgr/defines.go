...
// sciondConn holds the sciond connection and the current SIBRA service
// address. It is shared amongst the reservation manager and its resolvers.
// It is kept up to date by the reservation manager.
type sciondConn struct {
	sync.RWMutex
	// Srvc is the sciond service.
	Srvc sciond.Service
	// Conn is the sciond connector.
	Conn sciond.Connector
	// SvcSB is the current SIBRA service address.
	SvcSB *snet.Addr
}
