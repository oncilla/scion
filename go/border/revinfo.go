...
func (r *Router) fwdRevInfo(revInfo *path_mgmt.RevInfo, dstHost addr.HostAddr) {
	...
	if err != nil {
		log.Error("Down generating RevInfo Ctrl payload", "err", err)
		return
	}
	scpld, err := cpld.SignedPld(ctrl.NullSigner)
	if err != nil {
		log.Error("Down generating RevInfo signed Ctrl payload", "err", err)
		return
	}
	if err = r.genPkt(ctx.Conf.IA, *dstHost.(*addr.HostSVC), 0, srcAddr, scpld); err != nil {
		log.Error("Down generating RevInfo packet", "err", err)
	}
}
